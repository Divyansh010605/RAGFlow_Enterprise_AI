...
def chunk(text: str, size: int = 900, overlap: int = 120) -> list[str]:
    ...
    return [text[i:i + size] for i in range(0, len(text), max(1, size - overlap))] or [""]
...
def search(query: str, user_id: str, limit: int = 5) -> list[dict]:
    ...
    for row, words in zip(rows, docs):
        counts=Counter(words); bm25=sum((counts[t]/(len(words) or 1))*log((total+1)/(df[t]+1))+1 for t in q)
        dot=sum(q[t]*counts[t] for t in q); denom=sqrt(sum(v*v for v in q.values()))*sqrt(sum(v*v for v in counts.values()))
        score=bm25+(dot/denom if denom else 0)
        if score: scored.append({"chunk_id":row["id"],"document_id":row["document_id"],"filename":row["filename"],"ordinal":row["ordinal"],"text":row["text"],"score":round(score,3)})
